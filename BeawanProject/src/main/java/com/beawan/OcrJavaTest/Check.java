...
public class Check {
    ...
    private static final String POST_URL_ACCURATE = "https://aip.baidubce.com/rest/2.0/ocr/v1/accurate_basic?access_token=" + AuthService.getAuth();
    ...
    private static String post(String param) throws URISyntaxException, IOException {
        ...
        if (response.getStatusLine().getStatusCode() == 200) {
            ...
            try {
                ...
                List<Map<String, String>> wordList = (List<Map<String, String>>) map.get("words_result");
                if (!wordList.isEmpty()) {
                    if (wordList.size() > 1) {
                        StringBuilder sb = new StringBuilder();
                        for (Map<String, String> m : wordList) {
                            sb.append(m.get("words"));
                        }
                        return sb.toString();
                    }
                    return wordList.get(0).get("words");
                }
                return "";
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public static void main(String[] args) {
        String path = "D:\\pdf\\jpg\\result\\1545879312.jpg";
        try {
